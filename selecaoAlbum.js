...
function disallowCreation() {
  ...
  if (albumPics.length === 0) {
    ...
    Swal.fire({
      ...
      html: `Selecione as fotos que pretende manter na <a href="duplicados.html" id="linkDuplicados">pasta de duplicados</a>.`,
...
function replacePic(pictureID) {
  const pictureToReplace = document.querySelector(`img.${pictureID}`);
  if (pictureToReplace) {
    pictureToReplace.src = 'imagens/150x150transparente.png'; 
  }
}
